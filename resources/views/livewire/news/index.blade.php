        ...
        <x-slot name="header">
            <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
                <div class="flex md:flex-row flex-col md:justify-between justify-center items-center gap-4">
                    ...
                    @can('news.create')
                        <a href="{{ route('admin.news.create') }}" class="flex gap-1 bg-orange-600 hover:bg-orange-700 text-white font-semibold py-2 px-4 rounded-full transition-all duration-200">
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:px-0 px-4">
                    @forelse($news as $item)
                        ...
                        <div class="bg-white group overflow-hidden shadow-md flex flex-col rounded-t-2xl hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 relative">
                            <a href="{{ route('admin.news.profile', $item->id) }}" class="absolute inset-0 z-0"></a>
                            <div class="w-full h-[200px] border border-gray-300 rounded-t-2xl overflow-hidden bg-gray-100" x-data="{ loaded: false }">
                                @if($item->thumbnail_url)
                                    <img
                                        src="{{ $item->thumbnail_url }}"
                                        alt="{{ $item->title }}"
                                        class="w-full h-full object-cover rounded-t-2xl transition ease-out duration-200"
                            ...
                            <div class="p-4 flex flex-col h-[250px] group-hover:bg-orange-50 transition-all">
                                <a href="{{ route('admin.news.profile', $item->id) }}" class="flex-1">
                                    ...
                                    <span class="text-xs text-gray-500">Published on {{ $item->published_at ? $item->published_at->format('d M Y') : 'N/A' }}</span>
                                        ...
                                        <div class="flex items-center gap-2" onclick="event.stopPropagation()">
                                            @can('news.edit')
                                                <a
                                                    ...
                                                    title="Edit"
                                                    class="bg-blue-700/60 hover:bg-sky-600 text-white p-2 rounded-full transition-all"
                                                >
                                            ...
                                            @can('news.delete')
                                                <button
                                                    ...
                                                    class="bg-red-600/60 hover:bg-red-700 text-white p-2 rounded-full transition-all"
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
                    ...
                    @empty
                        <div class="col-span-full text-center text-gray-500 py-12 border-dashed border-2 border-gray-200 rounded-lg bg-white">
                            ...
                            <p class="mt-4">
                                <a href="{{ route('admin.news.create') }}" class="text-orange-600 hover:text-orange-700 font-medium">Create your first article</a>
                            </p>
                        </div>
                    @endforelse
                </div>
