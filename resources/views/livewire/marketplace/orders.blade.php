        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 flex flex-wrap justify-between items-center gap-3">
            <div class="flex items-center gap-3">
                ...
                @can('marketplace.edit')
                    <a href="{{ route('admin.marketplace.cashier') }}" class="text-green-600 font-medium hover:text-green-800 text-sm">{{ __('Cashier') }}</a>
                @endcan
                <h2 class="font-semibold md:text-xl text-2xl text-gray-800 leading-tight">
                    {{ __('Marketplace pickup') }}
                </h2>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="space-y-4">
                    ...
                    <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                        <div class="flex flex-wrap justify-between gap-2">
                            <div>
                                ...
                                <p class="text-sm text-orange-600 font-medium mt-1">{{ number_format($order->points_total) }} {{ __('pts') }}</p>
                            </div>
                            <div class="text-right">
                                ...
                                @if ($order->fulfilled_at)
                                    <p class="text-xs text-gray-500 mt-1">{{ $order->fulfilled_at->format('Y-m-d H:i') }}</p>
                                @endif
                            </div>
                        </div>
                ...
                @empty
                    <p class="text-center text-gray-600 py-12">{{ __('No orders match this filter.') }}</p>
                @endforelse
            </div>
